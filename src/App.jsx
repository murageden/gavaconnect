...
import { BrowserRouter, Routes, Route, Link as RouterLink, useParams, useNavigate, useLocation } from 'react-router-dom';
import { 
    ...
    Truck, CreditCard, Download, Settings, Send, XCircle, Loader2
} from 'lucide-react';

// --- DATA: API Catalog Data ---
...
// --- CONSTANT: Pagination Items Per Page ---
const ITEMS_PER_PAGE = 9; 

// --- COMPONENT: PageLoader ---
const PageLoader = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-white bg-opacity-75 transition-opacity duration-300">
        <div className="flex flex-col items-center">
            <Loader2 className="w-12 h-12 text-rose-800 animate-spin" />
            <p className="mt-4 text-lg font-semibold text-gray-700">Loading Page...</p>
        </div>
    </div>
);

// --- COMPONENT: Link (RouterLink with Loading State) ---
...
// --- COMPONENT: Clickable (Anchor Tag with Loading State) ---
...
const DetailPage = ({ setNotification, setIsLoading }) => {
...
const CatalogCard = ({ title, text, url, icon, setIsLoading }) => {
    return (
        <Link
            to={`/features/${url}`}
...
const Pagination = ({ itemsPerPage, totalItems, paginate, currentPage, setIsLoading }) => {
...
const Header = ({ setIsLoading }) => (
    <header className="sticky top-0 z-20 bg-rose-800 text-white shadow-xl w-full"> 
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex justify-between items-center">
            <Link to="/" setIsLoading={setIsLoading} className="text-xl font-bold text-white hover:text-rose-200 transition">
                Gava Connect
            </Link>
            ...
            <Clickable 
                ...
                setIsLoading={setIsLoading} 
                className="text-sm font-bold text-white px-4 py-2 rounded-lg bg-rose-900 hover:bg-rose-700 transition duration-150 cursor-pointer"
            >
                Developer Portal
            </Clickable>
        </div>
...
const CatalogView = ({ setIsLoading }) => {
...
// --- COMPONENT: Main (Layout and Routing) ---
const Main = () => {
    ...
    return (
        <div className="min-h-screen bg-gray-50 flex flex-col font-sans w-full"> 
            ...
            <Routes>
                <Route path="/" element={<CatalogView setIsLoading={setIsLoading} />} /> 
                
                <Route 
                    path="/features/:cardUrl" 
                    element={<DetailPage setNotification={setNotification} setIsLoading={setIsLoading} />} 
                />
            </Routes>
